...
pub fn get_status(state: State<'_, AppState>) -> Result<AppStatus, String> {
    ...
    let is_recording = *state.recording.lock().map_err(to_command_error)?;
...
pub fn start_recording(state: State<'_, AppState>) -> Result<AppStatus, String> {
    *state.recording.lock().map_err(to_command_error)? = true;
    get_status(state)
}

#[tauri::command]
pub fn stop_recording(state: State<'_, AppState>) -> Result<AppStatus, String> {
    *state.recording.lock().map_err(to_command_error)? = false;
    get_status(state)
}
...
pub fn generate_summary(state: State<'_, AppState>) -> Result<String, String> {
    let path = state.storage.write_today_summary().map_err(to_command_error)?;
